...
for (const version of ['v4r2', 'w5']) {
  test(`${version}: cancelling a fully prepared mint never submits a transaction`, async () => {
    ...
    let asked = false;
    try {
      await main(['mint', '0', '0'], { SEED_PHRASE: phrase, WALLET_VERSION: version }, async () => {
        asked = true;
